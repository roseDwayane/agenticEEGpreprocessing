...
- 下游效用即 MI-SMAC 的主結果（見 [[P_MI_SMAC]]）：50 次評估後在 35% 資料集上顯著優於冷啟動 SMAC、僅 7% 較差。
...
- **啟發了**：[[P_MI_SMAC]] — 相似度排序直接決定初始設計的配置來源；[[P_Similarity_Gating]] — 門控函數 g(s) 需要的相似度訊號即定義於此座標系上；本研究將其 EEG 化：以 band power、CSP eigen-spectrum 等訊號層統計量構造受試者 meta-features，作為跨受試者配置遷移與門控的座標系。
