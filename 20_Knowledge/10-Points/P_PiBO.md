...
## 啟發
- **被啟發**：[[P_MI_SMAC]] — 承接「用外部知識設定 BO 起點」的問題設定，但把知識載體從離散配置清單推廣為任意連續信念分布，並補上收斂理論。
- **啟發了**：[[P_LLM_Prior_Elicitation]] — π(x) 不必來自人類：LLM 依任務描述引出的分布可直接作為 πBO 先驗（本研究 LLM 臂的機制基礎）；其 β/n 衰減也為 [[P_Similarity_Gating]] 的「錯誤先驗安全底線」提供保證。
