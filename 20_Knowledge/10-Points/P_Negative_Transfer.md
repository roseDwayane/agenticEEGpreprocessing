...
- **啟發了**：[[P_Similarity_Gating]] — 負遷移風險（尤其是 surrogate 層面無法自我察覺的結構性錯估）直接創造了「以外部相似度訊號門控暖啟動強度、低相似度時退回冷啟動」的需求（causes 關係）；本研究據此將負遷移率列為正式實驗終點而非 nuisance。
