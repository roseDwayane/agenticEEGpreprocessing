...
## 啟發
- **被啟發**：[[P_SMAC]] — 直接建立在 SMAC 的 SMBO 框架與其可接受 initial design 的介面之上；[[P_Meta_Features]] — 46 個 meta-features 是配置推薦的座標系，整套方法的相似度計算依賴之。
- **啟發了**：[[P_PiBO]] — 從「離散配置清單」進化為「連續先驗分布」的知識注入；auto-sklearn 將本方法的 meta-learning 初始化內建為系統元件；[[P_Zero_Shot_Configuration_Transfer]] — 其 t 個 meta-建議配置在 SMBO 開始前先被評估，本身即是零樣本配置遷移的原型。
