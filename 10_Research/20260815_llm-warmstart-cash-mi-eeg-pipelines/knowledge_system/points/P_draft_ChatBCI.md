...
- **啟發了**：[[B_draft_llm-warmstart-cash-mi-eeg-pipelines]] — 本研究 F3（LLM 先驗引出介面：由 LLM 根據資料集／受試者描述產生 πBO 先驗）直接承接 ChatBCI 未來工作所呼籲的 LLM-driven HPO/NAS，但以「LLM 只做先驗注入、搜尋仍由古典 BO 執行」的閘控式設計回應其「低於 SoA、需引導」的教訓；[[P_draft_Subjects_as_Tasks]] — 提供了 LLM 知識與 EEG 管線之間缺失介面存在的證據。
