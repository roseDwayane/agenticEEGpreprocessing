...
parent:
parts: []
depends_on: [P_draft_Inter_Subject_Variability, P_draft_BCI_Calibration_Problem]
caused_by: []
causes: []
related_lines: []
related_planes: []
...
## 啟發
- **被啟發**：[[P_draft_MI_SMAC]] — datasets-as-tasks 的原型（meta-feature 相似度暖啟動）提供類比的 AutoML 半邊；[[P_draft_EEG_Reptile]] — 權重層級的 subjects-as-tasks 提供 EEG 半邊，其 few-shot 低天花板（43%→46%）並論證了配置層級遷移的必要性；[[P_draft_Inter_Subject_Variability]] 與 [[P_draft_BCI_Calibration_Problem]] — 供給任務異質性與成本動機。
- **啟發了**：[[B_draft_llm-warmstart-cash-mi-eeg-pipelines]] — 本研究全部設計（RQ1–RQ4、H1–H3、LOSO 先驗學習、相似度門控、零樣本遷移臂）皆由此概念展開；它是把 [[P_draft_Budget_Matched_Protocol]] 與 [[P_draft_Seeded_Baseline]] 的方法學防護接到 EEG 場域的載體。
