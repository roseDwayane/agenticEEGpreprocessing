...
parent:
parts: []
depends_on: [P_draft_Budget_Matched_Protocol]
caused_by: []
causes: []
related_lines: []
related_planes: []
...
## 啟發
- **被啟發**：[[P_draft_Budget_Matched_Protocol]] — seeded baseline 是該協議中決定性的對照臂，脫離預算對齊與成對統計則無從解讀；概念上亦上承 meta-learned 初始化傳統（FeurerEtAl2015，文中列為直接先例）。
- **啟發了**：[[B_draft_llm-warmstart-cash-mi-eeg-pipelines]] — 本研究 H3 把 seeded cold-start 列為內建對照臂（H3-H0：seeded 後暖啟動優勢消失；H3-H1：anytime-AUC 優勢存活，證明先驗形狀在 EEG 條件式空間中有超越首配置的引導價值），並據此把「增益化約為強預設」列為風險評估中的已知混淆。
