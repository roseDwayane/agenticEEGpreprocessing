---
schema_version: "1.0"
id: B_draft_llm-warmstart-cash-mi-eeg-pipelines
type: body
...
created: 2026-08-15
updated: 2026-08-15
modules: [F_draft_T4_EEG_Pipeline_Automation, F_draft_T3_CASH_Systems_Benchmarks, F_draft_T2_Warmstart_Transfer_BO, F_draft_T1_LLM_for_AutoML, F_draft_T5_CrossSubject_EEG]
...
- **M1 搜尋空間定義**（[[F_draft_T4_EEG_Pipeline_Automation]]）：5 階段條件式管線空間（resample → FIR/IIR band-pass → re-reference → epoch → CSP → classifier×HPs）
- **M2 搜尋引擎**（[[F_draft_T3_CASH_Systems_Benchmarks]]）：VolcanoML 式分解 + SMAC3/OpenBox
- **M3 知識注入**（[[F_draft_T2_Warmstart_Transfer_BO]]）：(a) LOSO 搜尋歷史先驗 (b) LLM 先驗（πBO 注入口）＋相似度門控
- **M4 LLM 知識源**（[[F_draft_T1_LLM_for_AutoML]]）：API 級模型的先驗引出 prompt 模板
- **M5 評估**（內建 GAP_003 協議）：anytime AUC、evaluations-to-target、seeded 對照、Wilcoxon＋Holm–Bonferroni
- **M6 零樣本遷移**（[[F_draft_T5_CrossSubject_EEG]]）：配置直接套用＋門控分析（H2）
...
- M1 空間大小 ↔ M2 是否分解（D_draft_Does_The_Optimizer_Matter 的裁決條件）
- [[P_draft_Inter_Subject_Variability]]（~30 倍離群偏差）→ M3 門控強度校準
