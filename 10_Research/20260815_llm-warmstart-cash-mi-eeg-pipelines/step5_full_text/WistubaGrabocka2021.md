...
Published as a conference paper at ICLR 2021
                                         ...
                                         F EW-S HOT BAYESIAN O PTIMIZATION                                                WITH          D EEP
                                         K ERNEL S URROGATES
                                          Martin Wistuba                                   Josif Grabocka
                                          IBM Research                                     University of Freiburg
                                          Dublin, Ireland                                  Freiburg, Germany
                                          martin.wistuba@ibm.com                           grabocka@cs.uni-freiburg.de

                                                                                      A BSTRACT

arXiv:2101.07667v1 [cs.LG] 19 Jan 2021
...
For example, Schilling et al. (2015) propose to use a neural network with task embedding and variable interactions. To obtain mean and variance predictions, the authors propose using an ensemble
of models. In contrast, Springenberg et al. (2016) use a Bayesian multi-task neural network. However, since training Bayesian neural networks is computationally intensive, Perrone et al. (2018)
...
model-agnostic meta-learning for few-shot learning (Finn et al., 2017). This will allow us to integrate all task-specific parameters out such that the model does not grow with the number of tasks.
...
A simpler solution for using the transfer learning idea in Bayesian optimization is initialization (Feurer et al., 2015; Wistuba et al., 2015a). The standard Bayesian optimization routine with a
...
3     P RELIMINARIES
3.1   BAYESIAN O PTIMIZATION

Bayesian optimization is an optimization method for computationally intensive black box functions
that consists of two main components, the surrogate model and the acquisition function. The surrogate model is a probabilistic model with mean µ and variance σ 2 , which tries to approximate the
...
score balances between exploration and exploitation. The following steps are carried out sequentially. First, the feasible solution that maximizes the acquisition function is evaluated. In this way a
new observation (x, f (x)) is obtained. Then, the surrogate model is updated based on the entirety of
all observations D. This sequence of steps is carried out until a previously defined convergence criterion occurs (e.g. exhaustion of the time budget). In Figure 1 we provide an example how Bayesian
...
3.2   G AUSSIAN P ROCESSES

We have a training set D of n observations, D = {(xi , yi )|i = 1, . . . , n}, where yi are noisy observations of the function values yi = f (xi ) + ε. We assume that the noise ε is additive independent
...
x∗ , the assumption is that their ground truth f∗ is jointly Gaussian with y
                                                               
                                   ...
                                   f∗                  KT∗ K∗∗
where
               Kn = k (X, X|θ) + σn2 I, K∗ = k (X, X∗ |θ) , K∗∗ = k (X∗ , X∗ |θ)                    (4)
for brevity. Then, the posterior predictive distribution has mean and covariance
                E [f∗ |X, y, X∗ ] = KT∗ K−1                             T  −1
                                         n y, cov [f∗ |X, X∗ ] = K∗∗ − K∗ Kn K∗                     (5)
...
learn the kernel function. They propose to use a neural network ϕ to transform the input x to a latent
representation which serves as input for the kernel function.
                            kdeep (x, x0 |θ, w) = k(ϕ(x, w), ϕ(x0 , w)|θ)                           (6)

4     F EW-S HOT BAYESIAN O PTIMIZATION
...
very few examples of the target task, we want to make reliable predictions. For each of the source
                                         (t) (t)                       (t)
tasks we have observations D(t) = {(xi , yi )}i=1...n(t) , where xi is a hyperparameter setting
      (t)                                   (t)
and yi is the noisy observations of f (t) (xi ), i.e. a validation score of a machine learning model
                                                                                  ST
on data set t. In the following we will denote the set of all data points by D := t=1 D(t) . Modelagnostic meta-learning (Finn et al., 2017) has become a popular choice for few-shot learning and
we propose to use an adaptation for Gaussian processes (Patacchiola et al., 2020) as a surrogate
model within the Bayesian optimization framework. The idea is simple. A deep kernel ϕ is used
...
over θ and w are dominated by their respective maximum likelihood estimates θ̂ and ŵ, we can
approximate the posterior predictive distribution by
                                Z                                                            
             p (f∗ |x∗ , D) =       p (f∗ |x∗ , θ, w) p (θ, w|D) dθ, w ≈ p f∗ |x∗ , D, θ̂, ŵ           (7)

We estimate θ̂ and ŵ by maximizing the log marginal likelihood for all tasks.
                                                             XT                        
      log p y(1) , . . . , y(T ) |X(1) , . . . , X(T ) , θ, w =   log p y(t) |X(t) , θ, w               (8)
                                                              t=1
                                                                 T                                
                                                                 X         T     −1
                                                            ∝−         y(t) K(t)
...
values for all T tasks, we can generate augmented tasks for each batch B = {xi , yi }i=1,...,b ∼ D(t) ,
...
task does not require normalization. This strategy is not possible for other hyperparameter optimization methods, since this either increases the training data set size even further and thus becomes
...
while not done do
   ...
   for bn times do
       Sample batch B = {(xi , yi )}i=1,...,b ∼ D(t) and scale labels y using l and u;
...
Published as a conference paper at ICLR 2021
                        ...
                        x1   x3    x5                       x1      x3   x5        x1    x7   x9

                               Replace one element
                                    at random                                        Merge two sets
                                                                                      at random
                        x1   x12   x5                                    x1   x3   x9
...
is available to the optimizer which are considered to be related. These source tasks t = 1, . . . , T
                                                                                (t)      (t)
are generated randomly with a ∼ U(0.1, 5) and b ∼ U(0, 2π). 50 examples (xi , f (t) (xi )) are
                                               (t)
available for each task t, whereby the points xi ∈ [−5, 5] are evenly spaced.
...
6   A DATA -D RIVEN I NITIALIZATION
...
find a set with I hyperparameter settings which minimizes this loss on the source tasks.
                                           T                         T
                                                                         min f˜(t) (x)
                                           X                           X
                   X(init) = arg min             L f (t) , X = arg min                                (12)
                                   X∈X I                                 X∈X I           x∈X
                                           t=1                                     t=1

The loss of a set of hyperparameters depends on the response function values for each of the elements. Since these are not available for every x, we approximate f (t) (x) with the prediction of the
...
normalized regret and is defined on the right side of Equation 12, where f˜(t) is derived from f (t) by
scaling it to [0, 1] range:
                                     ...
                                     f˜(t) (x) =     (t)     (t)
                                                   ...
                                                   fmax − fmin
        (t)       (t)
where fmax and fmin are the maximum and minimum of f (t) , respectively.
...
The best sets are then selected to be refined using an evolutionary algorithm. The algorithm randomly chooses either to mutate a set or to perform a crossover operation between two sets. When
...
standard and strong baseline (Snoek et al., 2012). We use a Matérn 5/2 kernel with ARD and rely
...
MetaBO Using the acquisition function instead of the surrogate model for transfer learning is another option. MetaBO is the state-of-the-art transfer acquisition function and was presented last year
...
Published as a conference paper at ICLR 2021
 ...
 Method                                 AdaBoost                       GLMNet                      SVM
                                   15      33       50           33       67       100       33      67       100
 R ANDOM                        4.87      3.02     2.16        0.85      0.40     0.29     2.01     1.12     0.83
 ...
 M ETA BO                       5.27      3.52     1.96       11.02     10.97    10.96    12.39    12.39    11.93
 ABLR                           4.56      1.44     1.24        1.77      0.50     0.40     1.81     1.23     0.84
 ABLR (WS)                      3.17      1.72     1.17        0.54      0.36     0.29     1.47     1.08     0.87
 M ULTI -H EAD GP S (WS)        6.78      3.45     1.80        2.83      2.80     2.68    11.20     9.82     8.72
...
where f˜(t) is the normalized response function (Equation 13), f˜min is the global minimum for task
                                                                  (t)

t, and xmin is the best discovered hyperparameter setting by the optimizer.

7.1      H YPERPARAMETER O PTIMIZATION

We conduct experiments on three different metadata sets and report the aggregated mean of normalized regrets in Table 1. The best results are in bold. Results that are not significantly worse than the
best are in italics. We determined the significance using the Wilcoxon signed rank test with a confidence level of 95%. Results are reported after every 33 trials of Bayesian optimization for GLMNet
...
and they confirmed that our results are obtained correctly. The problem is apparently that the Reinforcement Learning method does not work well for larger number of trials. We provide a longer
...
improvement. We analyzed the reason for this in more detail. We observed that the learned neural
   1
       Out-of-memory exception due to too large data.
...
Normalized Regret
                    ...
                    10
                             0              20           40               0              20            40                  0           20            40
                                     ...
                                     Multi-Head GPs                           Multi-Head GPs (warm start+fine-tune)                FSBO (warm start+fine-tune)
                                     Multi-Head GPs (warm start)              FSBO (warm start)
...
7.3                          DATA -D RIVEN WARM S TART

The use of a warm start is not the main contribution of this paper but it is interesting to explore further nonetheless. First of all, for some methods this is the only differentiating factor. We compare
our suggested warm start initialization with a random and a Latin hypercube sampling (LHS) initialization in Figure 4. Considering that GP (WS) always performed better than GP (LHS) in Table 1,
...
is that in this case the warm start might not have always found good settings as part of the initialization, it still explored areas in the search space close to the optimum. This would facilitate finding a
...
ACKNOWLEDGMENTS
This work has been supported by European Union’s Horizon 2020 research and innovation programme under grant number 951911 - AI4Media. Prof. Grabocka is thankful to the Eva Mayr-Stihl
Foundation for their generous research grant.

R EFERENCES
Rémi Bardenet, Mátyás Brendel, Balázs Kégl, and Michèle Sebag. Collaborative hyperparameter
...
Balázs Kégl and Róbert Busa-Fekete. Boosting products of base classifiers. In Proceedings of
...
Valerio Perrone, Rodolphe Jenatton, Matthias W. Seeger, and Cédric Archambeau. Scalable hy-
  ...
  2018, Montréal, Canada, pp. 6846–6856, 2018. URL http://papers.nips.cc/paper/
...
Nicolas Schilling, Martin Wistuba, Lucas Drumond, and Lars Schmidt-Thieme. Hyperparameter
  ...
  7-11, 2015, Proceedings, Part II, pp. 87–103, 2015. doi: 10.1007/978-3-319-23525-7\ 6. URL
...
Nicolas Schilling, Martin Wistuba, and Lars Schmidt-Thieme. Scalable hyperparameter optimiza-
  ...
  23, 2016, Proceedings, Part I, pp. 33–48, 2016. doi: 10.1007/978-3-319-46128-1\ 3. URL
...
Michael Volpp, Lukas P. Fröhlich, Kirsten Fischer, Andreas Doerr, Stefan Falkner, Frank Hutter,
...
Martin Wistuba, Nicolas Schilling, and Lars Schmidt-Thieme. Two-stage transfer surrogate model
 ...
 23, 2016, Proceedings, Part I, pp. 199–214, 2016. doi: 10.1007/978-3-319-46128-1\ 13. URL
...
Martin Wistuba, Nicolas Schilling, and Lars Schmidt-Thieme. Scalable gaussian process-based
 transfer surrogates for hyperparameter optimization. Mach. Learn., 107(1):43–78, 2018. doi: 10.
...
Normalized Regret
                    ...
                    100
                          0      20             40                0   25     50       75     100         0     25      50      75   100
                               Number of Trials                        Number of Trials                         Number of Trials
                                                 Random                  MetaBO            Multi-Head GPs (warm start)
                                                 GP (LHS)                ABLR              FSBO
                                               GP (warm start)

                                          Figure 5: FSBO is the best of all considered methods.

A                         H YPERPARAMETER O PTIMIZATION
...
In such a case, we assume that the default value was used. The GLMNet metadata has two continous hyperparameters: the elastic-net mixing parameter α ∈ [0, 1] and the regularization parameter
λ ∈ [2−10 , 210 ]. The SVM metadata has two mandatory hyperparameters and two conditional
                    2
                        https://github.com/boschresearch/MetaBO
...
Simple Regret
                ...
                0.00                                       0.00
                       0   500      1000    1500    2000          0          2000           4000                              0          20             40
                                 Iterations                                   Iterations                                               Number of Trials
                                                        MetaBO (Valid)                                                                  MetaBO (T=15)
                                                        MetaBO (Test)                                                                    MetaBO (T=50)
                                                        Random (Test, T=50)                                                              MetaBO (T=5..50)
                                                                                                                                         Random
...
et al. (2020).
                       ...
                       Method                           AdaBoost                       GLMNet                                          SVM
                                                   15      33          50         33     67   100                                 33     67       100
                       FSBO                    3.10        1.13       0.80    0.42         0.22    0.16                       0.79     0.51      0.36
                       FSBO (R EPTILE )        3.80        2.13       1.21    0.36         0.19    0.14                       1.48     0.89      0.72
...
2020). According to Wistuba et al. (2015b), this metadata was created using AdaBoost (Kégl &
...
The metadata was created by running a grid search on 50 different classification data sets, using classification accuracy as an objective. Compared to the other two metadata sets, this one is significantly
smaller. Its use is mainly motivated by comparing to MetaBO under the same circumstances as used
to evaluate MetaBO. For that reason, we do not use the leave-one-data-set-out cross-validation protocol but instead use the same fixed split created by Volpp et al. (2020).
Statistics about these metadata sets are provided in Table 3.

D                  M ETA -L EARNING WITH R EPTILE
In principle our few-shot surrogate can be combined with any model-agnostic meta-learning approach. In this section we compare our proposed meta-learning approach against Reptile (Nichol
et al., 2018), a first-order approximation of MAML. We use the same hyperparameters and data
augmentation as described in Algorithm 1. Reptile introduces a new hyperparameter, an outer learning rate. We set it to 0.1 and decay it linearly to 0. As summarized in Table 2, FSBO with the
...
Published as a conference paper at ICLR 2021
                    ...
                    AdaBoost                      GLMNet                SVM
                    DATA SET        RUNS   O PEN ML ID   RUNS    O PEN ML ID    RUNS
            ...
            BREAST- CANCER           108        3899     15954         3902      1105
                         ...
                         CHESS       108        3917     16960         3918      1459
                    COD - RNA        108        3918     20984         3950      1998
                   COIL 2000         108        9914     37635         6566      2702
           ...
           GERMAN - NUMER            108        9980     31961         9967      1492
                ...
                HOUSEVOTES           108       10101     66277         9976      1473
                        IJCNN 1      108      125923     35555         9978      1503
                      KR - VS - K    108      145847     29927         9980      2943
                  LED 7 DIGIT        108      145857     31180         9983       987
                       LETTER        108      145862     12954        10101      1871
            LYMPHOGRAPHY             108      145872     17835        14951      1765
                         MAGIC       108      145953     47439        34536      1499
                      MONK -2        108      145972     19972        34537      1489
                  PENDIGITS          108      145979     12979       145878      1244
                   PHONEME           108      146064     43785       146064      1435
                          PIMA       108
                           RING      108
                    SAHEART          108
                   SEGMENT           108
          ...
          WINEQUALITY- RED           108
                 WISCONSIN           108
                         YEAST       108
                         T OTAL     5400       T OTAL   804159        T OTAL    54418
